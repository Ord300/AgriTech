  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2">
        ...
        <Button type="button" onClick={handleSearch} disabled={searching || !query.trim()} className="gap-2">
          {searching ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}
          Rechercher
        </Button>
        <Button type="button" variant="outline" onClick={handleLocateMe} disabled={locating} className="gap-2 border-lime-400/20">
          {locating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Locate className="h-4 w-4 text-lime-400" />}
          Me localiser
        </Button>
      </div>
      ...
      <p className="text-[11px] text-muted-foreground">Clique sur la carte ou déplace le marqueur pour cibler l&apos;emplacement exact. La localisation sera utilisée pour la traçabilité.</p>

      {value && (
        <div className="rounded-lg bg-lime-400/10 p-2.5 text-xs">
